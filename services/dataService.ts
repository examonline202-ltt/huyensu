...
export const dataService = {
    ...
    async getExams(): Promise<ExamConfig[]> {
        // Sử dụng select * để tránh lỗi nếu cột start_time/end_time chưa được tạo trong DB
    ...
    // Lấy thông tin cơ bản đề thi (cho trang public)
    async getPublicExamInfo(examId: string): Promise<{ title: string, code: string, className: string } | null> {
        if (!examId) return null;
        try {
            const { data, error } = await supabase
                .from('exams')
                .select('title, code, class_name')
                .eq('id', examId)
                .single();
    ...
    async saveExam(exam: Partial<ExamConfig>, questions: Question[]): Promise<string> {
        ...
        if (isUpdate) {
            let { error: examError } = await supabase.from('exams').update(examPayload).eq('id', exam.id);
            
            // Xử lý lỗi nếu cột start_time/end_time chưa tồn tại hoặc lỗi cache schema
            if (examError && (
                examError.code === '42703' || 
                examError.message.includes('start_time') || 
                examError.message.includes('end_time') ||
                examError.message.includes('schema cache') ||
                examError.message.includes('Could not find the')
            )) {
                console.warn("DB schema mismatch (missing columns?), retrying without time fields...");
                delete examPayload.start_time;
                delete examPayload.end_time;
                const retry = await supabase.from('exams').update(examPayload).eq('id', exam.id);
                examError = retry.error;
            }

            if (examError) throw new Error("Lỗi cập nhật đề thi: " + examError.message);
        } else {
            const { id, ...payloadWithoutId } = examPayload;
            let { data, error: examError } = await supabase.from('exams').insert([payloadWithoutId]).select().single();
            
            // Xử lý lỗi tương thích cho Insert
            if (examError && (
                examError.code === '42703' || 
                examError.message.includes('start_time') || 
                examError.message.includes('end_time') ||
                ...
                const fallbackPayload = { ...payloadWithoutId };
                delete fallbackPayload.start_time;
                delete fallbackPayload.end_time;
                const retry = await supabase.from('exams').insert([fallbackPayload]).select().single();
                data = retry.data;
                examError = retry.error;
            }
