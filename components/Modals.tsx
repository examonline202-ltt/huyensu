...
import { MathRenderer, loadExternalLibs, copyToClipboard, parseWordSmart, generateSecurityCode, parseStudentImport, toLocalISOString } from '../utils/common';
...
export const PublishExamModal = ({ exam, onClose, onConfirm, onPlay, onCreateNew }: { exam: ExamConfig, onClose: () => void, onConfirm: (settings: any) => void, onPlay: () => void, onCreateNew: () => void }) => {
  const [settings, setSettings] = useState({
    ...
    // Use toLocalISOString to convert the stored UTC time to local input format "YYYY-MM-DDTHH:mm"
    startTime: exam.startTime ? toLocalISOString(exam.startTime) : '',
    endTime: exam.endTime ? toLocalISOString(exam.endTime) : '',
  ...
  const handleSave = async () => {
    // When saving, convert the local datetime-local string back to a full ISO string (UTC)
    // This ensures consistency between what the user sees (Local) and what is stored (UTC)
    const finalSettings = { ...settings };
    
    if (settings.startTime) {
        finalSettings.startTime = new Date(settings.startTime).toISOString();
    }
    if (settings.endTime) {
        finalSettings.endTime = new Date(settings.endTime).toISOString();
    }

    await onConfirm(finalSettings);
    onClose();
  };
